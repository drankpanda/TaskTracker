...
@router.get('/one-by-filters/', summary='Get one task or none by filter')
async def get_one_or_none_task_by_filter(request_body: RBTask = Depends()) -> Union[STask, dict]:
    result = await TaskDAO.find_one_or_none(**request_body.to_dict())
    if result is not None:
        return result
    return {'message': 'Task with given filters is not found!', 'filters': request_body.to_dict()}


@router.get('/{task_id}/', summary='Get one task or none by id')
async def get_one_or_none_task_by_id(task_id: int) -> Union[STask, dict]:
    result = await TaskDAO.find_one_or_none(id=task_id)
